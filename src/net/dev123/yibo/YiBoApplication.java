...
public class YiBoApplication extends Application {
	...
	private static final String TAG = YiBoApplication.class.getSimpleName();
	...
	public boolean isSliderEnabled() {
		return prefs.getBoolean(Constants.PREFS_KEY_USE_SLIDER, false);
	}
    ...
    private void initCachePath() {
    	...
    	String oldSdcardCachePath = null;
    	
    	String state = Environment.getExternalStorageState();
        if (Environment.MEDIA_MOUNTED.equals(state)) {
        	File sdcardPath = Environment.getExternalStorageDirectory();
    		oldSdcardCachePath = sdcardPath.getAbsolutePath() + File.separator + "yibo";
		...
		File newYiBoFile = new File(sdcardCachePath);
		File oldYiBoFile = new File(oldSdcardCachePath);
		if (oldYiBoFile.exists() && !newYiBoFile.exists()) {
			oldYiBoFile.renameTo(newYiBoFile);
		}
		if (oldYiBoFile.exists()) {
			oldYiBoFile.delete();
		}
    }
